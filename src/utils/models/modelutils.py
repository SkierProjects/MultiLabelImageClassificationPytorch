...
def get_learningRate_scheduler(optimizer, config=config):
    ...
    """
    return ReduceLROnPlateau(optimizer, mode='min', factor=config.learningrate_reducer_factor, patience=config.learningrate_reducer_patience, threshold=config.learningrate_reducer_threshold, verbose=True)